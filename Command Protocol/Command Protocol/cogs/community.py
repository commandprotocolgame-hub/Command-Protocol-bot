...
class SuggestionModal(discord.ui.Modal, title="Submit a Suggestion"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if channel_id:
            try:
                ...
                if channel:
                    ...
                    channel_embed.add_field(
                        name="Suggestion #",
                        value=f"`{suggestion_id:04d}`",
                        inline=True,
                    )

                    if isinstance(channel, discord.ForumChannel):
                        await channel.create_thread(
                            name=f"Suggestion #{suggestion_id:04d}",
                            embed=channel_embed,
                        )
                    else:
        ...
        embed.set_footer(
            text=f"Suggestion #{suggestion_id:04d}  •  COMMAND PROTOCOL"
        )
        ...
        if channel_id:
            try:
                ...
                if channel:
                    ...
                    channel_embed.add_field(
                        name="Severity",
                        value=f"`{severity_raw}`",
                        inline=True,
                    )

                    channel_embed.add_field(
                        name="Report #",
                        value=f"`{report_id:04d}`",
                        inline=True,
                    )
                    ...
                    if isinstance(channel, discord.ForumChannel):
                        await channel.create_thread(
                            name=f"Bug Report #{report_id:04d}",
                            embed=channel_embed,
                        )
                    else:
                        await channel.send(embed=channel_embed)
                ...
                log.error(f"Failed to post bug report to channel: {e}")
